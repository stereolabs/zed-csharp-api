...
namespace sl
{
    class GLViewer
    {
        ...
        public void init(CameraParameters param)
        {
            ...
            BBox_obj.init();
            BBox_obj.setDrawingType(PrimitiveType.Quads);
        ...
        public void update(ZEDMat image, ObjectsFrameSDK objects)
        {
            ...
            foreach (ObjectDataSDK obj in objects.objectData)
            {
                ...
                if (renderObject(obj))
                {
                    ...
                    float4 clr = generateColorClass(obj.id);
                    Vector3[] keypoints = obj.skeletonJointPosition;

                    if (showbbox) BBox_obj.addBoundingBox(bb_, clr);
                    ...
                    if (keypoints.Length > 0)
                    {
                        foreach(var limb in BODY_BONES)
                        {
                            ...
                            if (!float.IsNaN(norm_1) && norm_1 > 0 && !float.IsNaN(norm_2) && norm_2 > 0){
                                bones.addCylinder(new float3(kp_1.X, kp_1.Y, kp_1.Z), new float3(kp_2.X, kp_2.Y, kp_2.Z), clr);
                            }
                        }
                        for (int i = 0; i < (int)BODY_PARTS.LAST; i++)
                        {
                            ...
                            {
                                joints.addSphere(new float3(kp.X, kp.Y, kp.Z), clr);
                            }
                        }
                    }
                }
        ...
        sl.float4 generateColorClass(int idx) {
            ...
            color.x = colors[offset, 0];
            color.y = colors[offset, 1];
            color.z = colors[offset, 2];
            color.w = 1.0f;
            return color;
        }
    ...
    class Simple3DObject
    {
        ...
        public void addBoundingBox(List<Vector3> bbox, float4 base_clr)
        {
            ...
            float ratio = 1.0f / 5.0f;

            List<Vector3> bbox_ = new List<Vector3>();
            ...
            for (int i = 4; i < 8; i++)
            {
                var midd = bbox[i - 4] - (bbox[i - 4] - bbox[i]) * ratio;
                bbox_.Add(midd);
            }
            // generate BOTTOM FADE
            for (int i = 4; i < 8; i++)
            {
                var midd = bbox[i] + (bbox[i - 4] - bbox[i]) * ratio;
                bbox_.Add(midd);
            }
            // generate BOTTOM BOX
            for (int i = 4; i < 8; i++)
            ...
            for (int i = 0; i < bbox_.Count; i++)
            {
                vertices_.Add(bbox_[i].X);
                vertices_.Add(bbox_[i].Y);
                vertices_.Add(bbox_[i].Z);

                colors_.Add(base_clr.x);
                colors_.Add(base_clr.y);
                colors_.Add(base_clr.z);
                colors_.Add(((i > 3) && (i < 12)) ? 0 : base_clr.w); //fading
            }

            for (int i = 0; i < boxLinks.Length; i += 4)
            {
                indices_.Add(start_id + boxLinks[i]);
                indices_.Add(start_id + boxLinks[i + 1]);
                indices_.Add(start_id + boxLinks[i + 2]);
                indices_.Add(start_id + boxLinks[i + 3]);
            }

            for (int i = 0; i < boxLinks.Length; i += 4)
            {
                indices_.Add(start_id + 8 + boxLinks[i]);
                indices_.Add(start_id + 8 + boxLinks[i + 1]);
                indices_.Add(start_id + 8 + boxLinks[i + 2]);
                indices_.Add(start_id + 8 + boxLinks[i + 3]);
            }
        }
    ...
    public void addPoint(float3 pt, float4 clr)
        ...
        uint[] boxLinks = new uint[] { 0, 1, 5, 4, 1, 2, 6, 5, 2, 3, 7, 6, 3, 0, 4, 7 };
